...
public class CpsDisplay extends Module {
    ...
    private void draw(BooleanValue bool, String text, IntSupplier xSupplier, IntSupplier ySupplier) {
        if (bool.isToggled()) {
            mc.fontRendererObj.drawString(text, xSupplier.getAsInt() - mc.fontRendererObj.getStringWidth(text), ySupplier.getAsInt(), Color.getHSBColor((color.getInputToFloat() % 360) / 360.0f, 1.0f, 1.0f).getRGB(), true);
        }
    }
}
